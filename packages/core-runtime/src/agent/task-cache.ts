...
import {
  ...
  globalConfigManager,
} from '@omni/shared-runtime/env';
import { getDebug } from '@omni/shared-runtime/logger';
import { ifInBrowser, ifInWorker } from '@omni/shared-types/utils';
import { generateHashId } from '@omni/shared-types/utils';
import { replaceIllegalPathCharsAndSpace } from '@omni/shared-types/utils';
...
export class TaskCache {
  ...
  constructor(
    ...
    this.cacheFilePath =
      ifInBrowser || ifInWorker
        ? undefined
        : cacheFilePath ||
          join(getRunSubDir('cache'), `${this.cacheId}${cacheFileExt}`);
  ...
  loadCacheFromFile() {
    ...
    try {
      ...
      if (!version) {
        debug('no Omni version info, will not read cache from file');
        return undefined;
      }

      if (
        semver.lt(jsonData.OmniVersion, lowestSupportedOmniVersion) &&
        !jsonData.OmniVersion.includes('beta') // for internal test
      ) {
        console.warn(
      ...
      jsonData.OmniVersion = getCoreVersion(); // update the version
      return jsonData;
    } catch (err) {
      debug(
  ...
  flushCacheToFile(options?: { cleanUnused?: boolean }) {
    const version = getCoreVersion();
    if (!version) {
      debug('no Omni version info, will not write cache to file');
      return;
    }
