...
import type { TModelFamily } from '@omni/shared-runtime/env';
import { getPreferredLanguage } from '@omni/shared-runtime/env';
import {
  getZodDescription,
  getZodTypeName,
} from '@omni/shared-types/zod-schema-utils';
...
- "fill out the form" 鈫?Goal accomplished when all fields are filled. Do NOT submit the form.
- "click the login button" 鈫?Goal accomplished once clicked. Do NOT wait for page load or verify login success.
- "type 'hello' in the search box" 鈫?Goal accomplished when 'hello' is typed. Do NOT press Enter or trigger search.
- "select the first item" 鈫?Goal accomplished when selected. Do NOT proceed to checkout.
...
- **Keep it concise**: be no more than 1-2 sentences, focused on immediate, tangible next steps. (8鈥?2 words or Chinese characters for quick updates).
