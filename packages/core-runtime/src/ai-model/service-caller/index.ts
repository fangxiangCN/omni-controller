...
import {
  type IModelConfig,
  OMNI_LANGFUSE_DEBUG,
  OMNI_LANGSMITH_DEBUG,
  OMNI_MODEL_MAX_TOKENS,
  OPENAI_MAX_TOKENS,
  type TModelFamily,
...
import { assert, ifInBrowser } from '@omni/shared-types/utils';
...
async function createChatClient({
  ...
  if (
    openai &&
    globalConfigManager.getEnvConfigInBoolean(OMNI_LANGSMITH_DEBUG)
  ) {
    if (ifInBrowser) {
  ...
  if (
    openai &&
    globalConfigManager.getEnvConfigInBoolean(OMNI_LANGFUSE_DEBUG)
  ) {
    if (ifInBrowser) {
...
export async function callAI(
  ...
  const maxTokens =
    globalConfigManager.getEnvConfigValueAsNumber(OMNI_MODEL_MAX_TOKENS) ??
