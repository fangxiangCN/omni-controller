...
export type LocationResultType = z.infer<typeof LocationResult>;

// Export the input type - this is the inferred type from getLocationSchema()
...
export const isLocatorField = (field: any): boolean => {
...
export const dumpLocatorField = (field: any): string => {
  assert(
    isLocatorField(field),
    'field is not a Omni locator field',
  );
...
export const findAllLocatorField = (
  ...
  for (const fieldName of locatorFields) {
    ...
    if (fieldValue) {
      ...
      if (typeof fieldValue === 'string') {
        ...
        // Check if this field is actually a OmniLocationType object
