...
import {
  defaultRunDirName,
  getRunSubDir,
} from '@omni/shared-runtime/common';
import {
  OMNI_CACHE,
  OMNI_DEBUG_MODE,
  globalConfigManager,
} from '@omni/shared-runtime/env';
import { getRunningPkgInfo } from '@omni/shared-runtime/node';
import { assert, logMsg } from '@omni/shared-types/utils';
import {
  escapeScriptTag,
  ifInBrowser,
  ifInWorker,
  uuid,
} from '@omni/shared-types/utils';
 ...
 *   2. Legacy cacheId when cache is undefined (requires OMNI_CACHE env var)
...
export function writeDumpReport(
  ...
  if (process.env.OMNI_DEBUG_LOG_JSON) {
    ...
    logMsg(`Omni - dump file written: ${jsonPath}`);
  }
...
export function writeLogFile(opts: {
  ...
  const targetDir = getRunSubDir(type);
