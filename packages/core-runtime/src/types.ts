...
import type { NodeType } from '@omni/shared-types/constants';
import type { CreateOpenAIClientFn, TModelConfig } from '@omni/shared-runtime/env';
import type {
  BaseElement,
  LocateResultElement,
  Rect,
 ...
 * - For { base64: "..." } format: Left as-is (plain object)
 */
  ...
  serializeToFiles(basePath: string): void {
    ...
    for (const screenshot of screenshots) {
      if (screenshot.hasBase64()) {
        ...
        writeFileSync(imagePath, new Uint8Array(bytes));
...
export interface AgentOpt {
   ...
   * If omitted, the agent will also read `OMNI_REPLANNING_CYCLE_LIMIT` for backward compatibility.
   */
  ...
  /**
   * When set to true, Omni will use the target device's time (Android/iOS)
