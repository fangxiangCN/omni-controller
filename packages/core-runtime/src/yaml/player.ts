...
import { assert, ifInBrowser, ifInWorker } from '@omni/shared-types/utils';
import { type ZodTypeAny, z } from 'zod';

// previous defined yaml flow, as a helper
interface YamlFlowItemAIInput extends LocateOption {
  // previous version
...
interface YamlFlowItemAIKeyboardPress extends LocateOption {
  // previous version
...
interface YamlFlowItemAIScroll extends LocateOption, ScrollParam {
  // previous version
  // aiScroll: null;
...
import { getRunSubDir } from '@omni/shared-runtime/common';
...
export class ScriptPlayer<T extends YamlScriptEnv> {
  ...
  async playTask(taskStatus: ScriptPlayerTaskStatus, agent: Agent) {
    ...
    for (const flowItemIndex in flow) {
      ...
      if (
        'aiAct' in (flowItem as YamlFlowItemAIAction) ||
        'aiAction' in (flowItem as YamlFlowItemAIAction) ||
        'ai' in (flowItem as YamlFlowItemAIAction)
      ) {
        const actionTask = flowItem as YamlFlowItemAIAction;
      ...
      } else if ('aiAssert' in (flowItem as YamlFlowItemAIAssert)) {
        const assertTask = flowItem as YamlFlowItemAIAssert;
        const {
          aiAssert: prompt,
          errorMessage: msg,
          name,
      ...
      } else if ('aiWaitFor' in (flowItem as YamlFlowItemAIWaitFor)) {
        const waitForTask = flowItem as YamlFlowItemAIWaitFor;
      ...
      } else if ('sleep' in (flowItem as YamlFlowItemSleep)) {
        const sleepTask = flowItem as YamlFlowItemSleep;
        ...
        'javascript' in (flowItem as YamlFlowItemEvaluateJavaScript)
      ) {
        const evaluateJavaScriptTask =
          flowItem as YamlFlowItemEvaluateJavaScript;
        ...
        'logScreenshot' in (flowItem as YamlFlowItemLogScreenshot) ||
        'recordToReport' in (flowItem as YamlFlowItemLogScreenshot)
      ) {
        const recordTask = flowItem as YamlFlowItemLogScreenshot;
        const title =
      ...
      } else if ('aiInput' in (flowItem as YamlFlowItemAIInput)) {
        // may be input empty string ''
        const {
          aiInput,
          value: rawValue,
          ...inputTask
        } = flowItem as YamlFlowItemAIInput;
        ...
        'aiKeyboardPress' in (flowItem as YamlFlowItemAIKeyboardPress)
      ) {
        const { aiKeyboardPress, ...keyboardPressTask } =
          flowItem as YamlFlowItemAIKeyboardPress;
      ...
      } else if ('aiScroll' in (flowItem as YamlFlowItemAIScroll)) {
        const { aiScroll, ...scrollTask } =
          flowItem as YamlFlowItemAIScroll;
