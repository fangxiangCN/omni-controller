...
import { getReportFileName, printReportMsg } from '@omni/core/agent';
import { getReportTpl } from '@omni/core/utils';
import { getMidsceneRunSubDir } from '@omni/shared/common';
import {
  escapeScriptTag,
...
class MidsceneReporter implements Reporter {
  ...
  private async updateReport(testData: ReportDumpWithAttributes) {
    ...
    this.writeQueue = this.writeQueue.then(async () => {
      ...
      if (!tpl) {
        throw new Error(
          'Report template not found. Ensure @omni/core is built correctly.',
        );
      }
