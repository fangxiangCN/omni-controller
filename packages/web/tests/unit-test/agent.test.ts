...
import type { GroupedActionDump } from '@omni/core';
import { Agent as PageAgent } from '@omni/core/agent';
import { globalConfigManager } from '@omni/shared/env';
import { beforeEach, describe, expect, it, vi } from 'vitest';

declare const __VERSION__: string;
// Mock only the necessary parts to avoid side effects
vi.mock('@omni/core/utils', async () => {
  const actual = await vi.importActual('@omni/core/utils');
  return {
    ...actual,
...
vi.mock('@omni/shared/logger', () => ({
  getDebug: vi.fn(() => vi.fn()),
  logMsg: vi.fn(),
}));

vi.mock('@omni/core', async () => {
  const actual = await vi.importActual('@omni/core');
  return {
    ...actual,
