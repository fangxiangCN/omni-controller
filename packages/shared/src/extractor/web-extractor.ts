...
export function collectElementInfo(
  ...
  if (isFormElement(node)) {
    ...
    const nodeHashId = generateHash(node, valueContent, rect);
  ...
  if (isButtonElement(node)) {
    ...
    const nodeHashId = generateHash(node, content, rect);
    const elementInfo: WebElementInfo = {
      id: nodeHashId,
      indexId: indexId++,
      nodeHashId,
  ...
  if (isImgElement(node)) {
    const attributes = getNodeAttributes(node, currentWindow);
    const nodeHashId = generateHash(node, '', rect);
    const elementInfo: WebElementInfo = {
      id: nodeHashId,
      indexId: indexId++,
      nodeHashId,
  ...
  if (isTextElement(node)) {
    ...
    const nodeHashId = generateHash(node, text, rect);
    const elementInfo: WebElementInfo = {
      id: nodeHashId,
      indexId: indexId++,
      nodeHashId,
