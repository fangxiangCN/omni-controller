...
import { generateElementByRect } from '@omni/shared/extractor';
import { getDebug } from '@omni/shared/logger';
import { _keyDefinitions } from '@omni/shared/us-keyboard-layout';
import { assert, logMsg, uuid } from '@omni/shared/utils';
...
export function printReportMsg(filepath: string) {
  if (globalConfigManager.getEnvConfigInBoolean(OMNI_REPORT_QUIET)) {
    return;
  }
  logMsg(`Omni - report file updated: ${filepath}`);
}
...
export function generateCacheId(fileName?: string): string {
  ...
  if (!taskFile) {
    taskFile = uuid();
    console.warn(
      'Omni - using random UUID for cache id. Cache may be invalid.',
    );
  }
...
export const getCoreVersion = (): string => {
