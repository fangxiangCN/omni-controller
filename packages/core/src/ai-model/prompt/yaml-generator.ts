...
import { YAML_EXAMPLE_CODE } from '@omni/shared/constants';
...
export const generateYamlTest = async (
  ...
  try {
    ...
    const prompt: ChatCompletionMessageParam[] = [
      {
        role: 'system',
        content: `You are an expert in Omni.js YAML test generation. Generate clean, accurate YAML following these rules: ${YAML_EXAMPLE_CODE}`,
      },
      {
        role: 'user',
        content: `Generate YAML test for Omni.js automation from recorded browser events.

Event Summary:
${JSON.stringify(yamlSummary, null, 2)}

Convert events:
- navigation â†?target.url
- click â†?aiTap with element description
- input â†?aiInput with value and locate
- scroll â†?aiScroll with appropriate direction
