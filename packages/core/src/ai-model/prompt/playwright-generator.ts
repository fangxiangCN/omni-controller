...
import { PLAYWRIGHT_EXAMPLE_CODE } from '@omni/shared/constants';
import type { IModelConfig } from '@omni/shared/env';
...
export const generatePlaywrightTest = async (
  ...
  // Create prompt text
  const promptText = `Generate a Playwright test using @omni/web/playwright that reproduces this recorded browser session. The test should be based on the following events and follow the structure of the example provided. Make the test descriptive with appropriate assertions and validations.
...
4. Implement a test that uses Omni AI methods (aiTap, aiInput, aiAssert, etc.)
...
Important: Return ONLY the raw Playwright test code. Do NOT wrap the response in markdown code blocks (no \`\`\`typescript, \`\`\`javascript or \`\`\`). Start directly with the code content.`;
  ...
  const systemPrompt = `You are an expert test automation engineer specializing in Playwright and Omni. 
Your task is to generate a complete, executable Playwright test using @omni/web/playwright that reproduces a recorded browser session.
