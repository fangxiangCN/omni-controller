...
import { getReportFileName, printReportMsg } from '@omni/core-runtime/agent';
import { getReportTpl } from '@omni/core-runtime/utils';
import { getMidsceneRunSubDir } from '@omni/shared-runtime/common';
import {
  escapeScriptTag,
...
class MidsceneReporter implements Reporter {
  ...
  private async updateReport(testData: ReportDumpWithAttributes) {
    ...
    this.writeQueue = this.writeQueue.then(async () => {
      ...
      if (!tpl) {
        throw new Error(
          'Report template not found. Ensure @omni/core-runtime is built correctly.',
        );
      }
