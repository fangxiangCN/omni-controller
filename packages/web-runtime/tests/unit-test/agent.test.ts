...
import type { GroupedActionDump } from '@omni/core-runtime';
import { Agent as PageAgent } from '@omni/core-runtime/agent';
import { globalConfigManager } from '@omni/shared-runtime/env';
import { beforeEach, describe, expect, it, vi } from 'vitest';

declare const __VERSION__: string;
// Mock only the necessary parts to avoid side effects
vi.mock('@omni/core-runtime/utils', async () => {
  const actual = await vi.importActual('@omni/core-runtime/utils');
  return {
    ...actual,
...
vi.mock('@omni/shared-runtime/logger', () => ({
  getDebug: vi.fn(() => vi.fn()),
  logMsg: vi.fn(),
}));

vi.mock('@omni/core-runtime', async () => {
  const actual = await vi.importActual('@omni/core-runtime');
  return {
    ...actual,
