...
/**
 * Check if a field is a Omni locator field
 * Checks for either:
 * 1. Omni_location_field_flag in shape (result schema)
 * 2. prompt field in shape (input schema)
 */
export function isLocatorField(field: unknown): boolean {
 ...
 * @param locatorTypeDescription - Optional description for OmniLocation fields (used by core)
 */
export function getZodTypeName(
  ...
  if (fieldTypeName === 'ZodObject') {
    // Check if this is a Omni locator field
...
export function getZodDescription(field: z.ZodTypeAny): string | null {
  ...
  // Check for OmniLocation fields and add description
  if (isLocatorField(actualField)) {
