...
export function StatusBar() {
  ...
  const isConnecting = Boolean(connectingDeviceId && connectingDeviceId === activeDeviceId)
  const isConnected = Boolean(activeDevice) && !isConnecting
  const modelName = process.env.MIDSCENE_MODEL_NAME || 'Unset'
  const taskCount = taskState.status === 'running' ? 1 : 0
  const resolution = activeDevice?.size
    ? `${activeDevice.size.width}×${activeDevice.size.height}`
    : '-'
  const osVersion = activeDevice?.osVersion || '-'
  ...
  return (
    <div className="status-bar">
      ...
      <div className="status-section">{resolution}</div>
      <div className="status-section">{osVersion}</div>
